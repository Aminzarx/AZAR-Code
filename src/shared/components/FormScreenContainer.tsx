...
type Props = {
  ...
  /** Screen title shown in the persistent header. */
  headerTitle?: string
  /**
   * Renders an explicit back control in the header (§16 nav audit —
   * MainNavigator sets headerShown:false everywhere, so without this
   * a screen has no in-UI way to leave besides the OS swipe/hardware
   * back gesture). Typically `() => navigation.goBack()`; a screen with
   * its own exit rules (e.g. useUnsavedChangesGuard) keeps working
   * unchanged since that guard intercepts goBack() itself. Showing the
   * header no longer requires `onSave` — pass just `onBack` for a
   * read-only screen's back button alone.
   */
  onBack?: () => void
  /**
        ...
        <View style={styles.header}>
          ...
          {onSave ? (
            <Pressable
              ...
              // Guarding inside the handler (not just via the `disabled`
              // prop) keeps this reliably no-op regardless of how the
              // underlying gesture-handler Pressable wires up
              // disabled-state press suppression at the native/gesture level.
              onPress={() => {
                if (!isDisabled) {
                  onSave()
                }
            ...
            >
              ...
              {isSaving ? (
                ...
                <View style={styles.savedFlash}>
                  ...
                  <Text style={[theme.typography('labelMd'), styles.savedFlashLabel]}>
                    ذخیره شد
                  </Text>
                </View>
              ) : (
            ...
            <Text
              style={[theme.typography('titleSm'), styles.headerTitle, styles.standaloneTitle]}
              numberOfLines={1}
              ellipsizeMode="tail"
            >
              {headerTitle}
